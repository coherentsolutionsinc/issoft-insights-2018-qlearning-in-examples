...
public class QModel {
    ...
    public void update(Player agent, Player enemy) {
        State state = new State(agent.getLocation(), enemy.getLocation());
        double score = getScore(state);
        score = score + ALPHA * (getReward(state) + GAMMA * bestScore(agent, enemy) - score);
        q.put(state, score);
    }
